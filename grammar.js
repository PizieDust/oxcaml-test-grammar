...
// CR-someday mkeys: Make it so that hash numbers must either
...
module.exports = grammar({
	...
	supertypes: ($) => [
		...
		$._tuple_type,
		$._type,
		// CR-someday tkoukpari for tkoukpari: add these back
		// $._simple_expression,
		// $._expression,
	...
	conflicts: ($) => [[$._parameter_type, $._tuple_type]],

	rules: {
		compilation_unit: ($) => seq(optional($.shebang), optional($._structure)),
				...
				seq(
					...
					repeat(
						choice(
							seq(
								repeat(";;"),
								choice($._structure_item, $.toplevel_directive),
							),
			...
			prec.right(
				seq(
					choice(
						...
						//    let local_ x
						//    let local_ ( + )
						seq($._mode_expr_legacy, field("pattern", $._value_name)),
					),
					optional(
						choice(
							$.at_mode_expr,
		...
		parameter: ($) =>
			choice(
				...
				//    ?(x : 'a. 'a)
				//    ?(x : t = y)
				// CR-someday mkeys: Currently, this rule allows default values on labeled
			...
			seq(
				...
				choice(
					seq(
						...
						optional(seq("=", $._type_binding_body)),
		...
		type_synonym: ($) => seq($._type, "="),

		_type_params: ($) =>
			choice(
				$._type_param,
				parenthesize(
					sep1(",", seq($._type_param, optional($._jkind_annotation))),
				),
			),
		...
		_type_equation: ($) =>
			seq(choice(seq("=", optional("private")), ":="), $._type),
		...
		field_declaration: ($) =>
			seq(
				optional(choice("mutable", $.modality_legacy)),
				$._field_name,
				$._maybe_polymorphic_typed,
				optional($.atat_modality_expr),
			),
		...
		class_definition: ($) =>
			seq("class", optional($._attribute), sep1("and", $.class_binding)),

		class_binding: ($) =>
			prec.right(
				seq(
					optional("virtual"),
					optional(seq("[", sep1(",", $._type_param), "]")),
		...
		_simple_module_type: ($) =>
			choice($.module_type_path, $.signature, $.parenthesized_module_type),
		...
		signature: ($) => seq("sig", optional($._signature), "end"),
		...
		constrain_module: ($) =>
			seq("module", $.module_path, choice("=", ":="), $.extended_module_path),
		...
		module_type_of: ($) => seq("module", "type", "of", $._module_expression),
		...
		parenthesized_module_type: ($) => seq(parenthesize($._module_type)),
		...
		structure: ($) => seq("struct", optional($._structure), "end"),
		...
		typed_module_expression: ($) =>
			parenthesize(seq($._module_expression, $._module_typed)),
		...
		instantiated_class_type: ($) =>
			seq("[", sep1(",", $._type), "]", $.class_type_path),
		...
		inheritance_specification: ($) =>
			seq("inherit", $._simple_class_type, repeat($.item_attribute)),
		...
		type_parameter_constraint: ($) =>
			seq("constraint", $._type, "=", $._type, repeat($.item_attribute)),

		let_open_class_type: ($) =>
			prec.right(
				PREC.match,
				seq("let", $.open_module, "in", field("body", $._simple_class_type)),
			),

		class_function_type: ($) =>
			prec.right(PREC.seq, seq($._class_parameter_type, "->", $._class_type)),
		...
		instantiated_class: ($) => seq("[", sep1(",", $._type), "]", $.class_path),

		typed_class_expression: ($) =>
			seq(parenthesize(seq($._class_expression, $._class_typed))),
		...
		class_initializer: ($) =>
			seq("initializer", $._sequence_expression, repeat($.item_attribute)),
		...
		parenthesized_class_expression: ($) =>
			seq(parenthesize($._class_expression)),
		...
		_type_with_modes: ($) =>
			choice(
				seq($._tuple_type, optional($.at_mode_expr)),
				$.function_type,
				$.aliased_type,
			),

		_typed_with_optional_modes: ($) => seq(":", $._type_with_modes),
		...
		polymorphic_type_with_optional_modes: ($) =>
			seq($._type_variables, ".", $._type_with_modes),

		_polymorphic_typed_with_optional_modes: ($) =>
			seq(":", $.polymorphic_type_with_optional_modes),

		// Does not allow modes
		_maybe_polymorphic_typed: ($) => choice($._typed, $._polymorphic_typed),
		...
		polymorphic_type: ($) => seq($._type_variables, ".", $._type),

		_coerced: ($) => seq(":>", $._type),

		// One or more abstract types. Does not allow layout annotations. Example:
		//    type t s
		_abstract_types: ($) => seq("type", repeat1($._type_constructor)),
		...
		_abstract_types_with_optional_jkind_annotations: ($) =>
			seq(
				...
				repeat1(
					choice(
						$._type_constructor,
						parenthesize(seq($._type_constructor, $._jkind_annotation)),
					),
				),
			),
		...
		_abstract_type_with_jkind_annotation: ($) =>
			seq("type", $._type_constructor, $._jkind_annotation),
		...
		type_variable_with_jkind_annotation: ($) =>
			parenthesize(seq($.type_variable, $._jkind_annotation)),

		type_wildcard_with_jkind_annotation: ($) =>
			parenthesize(seq($._type_wildcard, $._jkind_annotation)),

		_type_variable_with_optional_jkind_annotation: ($) =>
			choice($.type_variable, $.type_variable_with_jkind_annotation),
		_type_wildcard_with_optional_jkind_annotation: ($) =>
			choice($._type_wildcard, $.type_wildcard_with_jkind_annotation),

		_jkind_annotation: ($) => seq(":", $.jkind),

		_simple_jkind: ($) =>
			choice($.jkind_default, $.jkind_abbreviation, seq("(", $.jkind, ")")),
		...
		jkind_abbreviation: ($) => seq($._identifier),

		jkind_mod: ($) => seq($.jkind, "mod", $._mode_expr),

		jkind_with: ($) =>
			seq($.jkind, "with", $._simple_type, optional($.atat_modality_expr)),

		jkind_kind_of: ($) => seq("kind_of", $._simple_type),

		jkind_product: ($) =>
			seq($._simple_jkind, repeat1(seq("&", $._simple_jkind))),
		...
		_type: ($) => choice($._tuple_type, $.function_type, $.aliased_type),
		...
		_parameter_type: ($) =>
			prec(
				// Needs to be the same as _tuple_type to trigger
				// [_parameter_type]/[_tuple_type] conflict
				PREC.seq,
				choice(
					$.typed_label,
					...
					// We can't have a tuple_type_with_leading_label here because any
					// leading label should be parsed as the label for the parameter, not
		...
		// unlabeled
		_tuple_type_without_leading_label: ($) =>
			choice($._simple_type, $.proper_tuple_type),

		// A tuple type of at least two elements such that the first element is
		// labeled
		tuple_type_with_leading_label: ($) =>
			prec(PREC.seq, seq($._label_name, ":", $.proper_tuple_type)),

		// A tuple type of at least two elements such that the first element is
		// unlabeled
		proper_tuple_type: ($) =>
			seq($._simple_type, "*", sep1("*", $._tuple_type_element)),

		_tuple_type_element: ($) =>
			prec(PREC.prod, seq(optional(seq($._label_name, ":")), $._simple_type)),
		...
		constructed_type: ($) =>
			prec(
				...
				seq(
					...
					choice($.type_constructor_path, $.unboxed_type_constructor_path),
				),
			),
				...
				parenthesize(
					sep2(
						...
						choice(
							...
							//    'a : value
							seq($.type_variable, $._jkind_annotation),
							// A type wildcard with a layout annotation. Example:
							//    _ : value
							seq($._type_wildcard, $._jkind_annotation),
						),
					),
				),
			),
		...
		tag_specification: ($) =>
			seq($.tag, optional(seq("of", optional("&"), sep1("&", $._type)))),

		package_type: ($) =>
			parenthesize(seq("module", optional($._attribute), $._module_type)),
		...
		object_type: ($) =>
			seq(
				...
				optional(
					choice(
						seq(
							sep1(";", choice($.method_type, $._simple_type)),
		...
		method_type: ($) => seq($._method_name, $._maybe_polymorphic_typed),

		hash_type: ($) =>
			prec(
				PREC.hash,
				seq(optional($._constructed_type_arguments), "#", $.class_type_path),
			),

		parenthesized_type: ($) => parenthesize($._type),

		unboxed_tuple_type: ($) => seq("#(", $._tuple_type, ")"),
		...
		_simple_expression: ($) =>
			choice(
				...
				$.iarray_expression,
				$.record_expression,
				$.unboxed_record_expression,
				$.unboxed_tuple_expression,
				$.prefix_expression,
				$.hash_expression,
				$.field_get_expression,
				$.unboxed_field_get_expression,
		...
		_sequence_expression: ($) => choice($._expression, $.sequence_expression),

		typed_expression: ($) =>
			parenthesize(seq($._sequence_expression, $._typed_with_optional_modes)),

		// CR-someday tdelvecchio: Consider flattening expressions like a, b, c.
			...
			prec(
				...
				choice(
					...
					seq(
						"~",
						$._label_name,
						optional(seq(token.immediate(":"), $._simple_expression)),
					),
		...
		list_expression: ($) =>
			seq(
				"[",
				optional(
					choice(seq(sep1(";", $._expression), optional(";")), $.comprehension),
				),
				"]",
			),
		...
		comprehension: ($) => seq($._expression, repeat1($.comprehension_clause)),

		comprehension_clause: ($) =>
			choice(
				seq("for", sep1("and", $.comprehension_iterator)),
		...
		unboxed_tuple_expression: ($) => seq("#(", $.product_expression, ")"),
		...
		sign_expression: ($) =>
			prec(
				PREC.neg,
				seq(field("operator", $.sign_operator), field("right", $._expression)),
			),
		...
		field_get_expression: ($) =>
			prec.left(PREC.dot, seq($._simple_expression, ".", $.field_path)),
		...
		do_clause: ($) => seq("do", optional($._sequence_expression), "done"),
		...
		match_expression: ($) =>
			prec.right(
				PREC.match,
				seq(
					choice(seq("match", optional($._attribute)), $.match_operator),
					$._sequence_expression,
					"with",
					$._match_cases,
				),
		...
		_match_cases: ($) =>
			prec.right(seq(optional("|"), sep1("|", $.match_case))),
		...
		guard: ($) => seq("when", $._sequence_expression),
		...
		let_expression: ($) =>
			prec.right(
				PREC.match,
				seq($.value_definition, "in", $._sequence_expression),
			),

		coercion_expression: ($) =>
			parenthesize(seq($._sequence_expression, optional($._typed), $._coerced)),

		assert_expression: ($) =>
			prec.left(
				PREC.app,
				seq("assert", optional($._attribute), $._simple_expression),
			),

		lazy_expression: ($) =>
			prec.left(
				PREC.app,
				seq("lazy", optional($._attribute), $._simple_expression),
			),
		...
		let_open_expression: ($) =>
			prec.right(
				PREC.match,
				seq("let", $.open_module, "in", field("body", $._sequence_expression)),
			),
		...
		stack_expression: ($) => seq("stack_", $._expression),

		mode_legacy_expression: ($) => seq($.mode_legacy, $._sequence_expression),

		exclave_legacy_expression: ($) =>
			seq($.exclave_legacy, $._sequence_expression),

		new_expression: ($) => seq("new", optional($._attribute), $.class_path),

		object_copy_expression: ($) =>
			seq("{<", sep(";", $.instance_variable_expression), optional(";"), ">}"),

		instance_variable_expression: ($) =>
			seq($._instance_variable_name, optional(seq("=", $._expression))),

		method_invocation: ($) =>
			prec.right(PREC.hash, seq($._simple_expression, "#", $._method_name)),

		object_expression: ($) =>
			seq(
				"object",
				optional($._attribute),
				optional(parenthesize(seq($._pattern, optional($._typed)))),
		...
		parenthesized_expression: ($) =>
			choice(
				seq("begin", optional($._attribute), $._sequence_expression, "end"),
		...
		_simple_pattern: ($) =>
			choice(
				...
				$.record_pattern,
				$.unboxed_record_pattern,
				$.unboxed_tuple_pattern,
				$.list_pattern,
				$.array_pattern,
				$.iarray_pattern,
				$.local_open_pattern,
		...
		_pattern: ($) => choice($._pattern_no_exception, $.exception_pattern),

		alias_pattern: ($) =>
			prec.left(PREC.match, seq($._pattern, "as", $._value_name)),
		...
		_simple_maybe_polymorphic_pattern_with_optional_modes: ($) =>
			choice($._simple_pattern, $.polymorphic_typed_pattern),

		pattern_with_modes: ($) => parenthesize(seq($._pattern, $.at_mode_expr)),

		pattern_with_modes_legacy: ($) =>
			parenthesize(
				seq($._mode_expr_legacy, $._pattern, optional($.at_mode_expr)),
			),

		or_pattern: ($) => prec.left(PREC.seq, seq($._pattern, "|", $._pattern)),
		...
		tag_pattern: ($) => prec.right(PREC.app, seq($.tag, $._pattern)),

		polymorphic_variant_pattern: ($) => seq("#", $.type_constructor_path),

		// CR-someday tdelvecchio: Consider flattening patterns like a, b, c.
		tuple_pattern: ($) =>
			prec.left(
				PREC.prod,
				seq(
			...
			prec(
				...
				choice(
					...
					seq(
						"~",
						$._label_name,
						optional(seq(token.immediate(":"), $._simple_pattern)),
					),
					seq("~", "(", $._label_name, $._typed, ")"),
				),
			),
		...
		unboxed_tuple_pattern: ($) => seq("#(", $.tuple_pattern, ")"),

		field_pattern: ($) =>
			seq($.field_path, optional($._typed), optional(seq("=", $._pattern))),

		list_pattern: ($) =>
			prec.left(
				seq("[", optional(seq(sep1(";", $._pattern), optional(";"))), "]"),
			),
		...
		range_pattern: ($) =>
			prec(PREC.dot, seq($._signed_constant, "..", $._signed_constant)),

		lazy_pattern: ($) =>
			prec(PREC.hash, seq("lazy", optional($._attribute), $._pattern)),
		...
		exception_pattern: ($) =>
			seq("exception", optional($._attribute), $._pattern),
		...
		quoted_extension: ($) =>
			seq("{%", $.attribute_id, optional(/\s+/), $._quoted_string, "}"),
		...
		_constant: ($) => choice($._value_constant, $.unboxed_constant),

		_signed_value_constant: ($) => choice($._value_constant, $.signed_number),

		signed_unboxed_constant: ($) => seq(/[+-]/, HASH_NUMBER),

		_signed_constant: ($) =>
			choice(
				$._signed_value_constant,
				choice($.unboxed_constant, $.signed_unboxed_constant),
			),
		...
		conversion_specification: ($) =>
			token(
				seq(
					...
					optional(/[1-9][0-9]*|\*/),
					optional(/\.([0-9]*|\*)/),
					choice(/[diunlLNxXosScCfFeEgGhHbBat!%@,]/, /[lnL][diuxXo]/),
				),
			),
		...
		unit: ($) =>
			choice(seq("(", ")"), seq("begin", optional($._attribute), "end")),
		...
		prefix_operator: ($) =>
			token(
				choice(
					seq(
						"!",
						choice(optional(/[#!$%&*+\-./:<>?@^|~]/), repeat2(HASH_OP_CHAR)),
					),
		...
		pow_operator: ($) =>
			token(choice("lsl", "lsr", "asr", seq("**", repeat(OP_CHAR)))),

		mult_operator: ($) =>
			token(
				choice("mod", "land", "lor", "lxor", seq(/[*/%]/, repeat(OP_CHAR))),
			),
		...
		concat_operator: ($) => token(seq(/[@^]/, repeat(OP_CHAR))),
		...
		indexing_operator: ($) => token(seq(/[!$%&*+\-/:=>?@^|]/, repeat(OP_CHAR))),

		indexing_operator_path: ($) => path($.module_path, $.indexing_operator),

		let_operator: ($) => token(seq("let", /[$&*+\-/<=>@^|]/, repeat(OP_CHAR))),

		let_and_operator: ($) =>
			token(seq("and", /[$&*+\-/<=>@^|]/, repeat(OP_CHAR))),

		match_operator: ($) =>
			token(seq("match", /[$&*+\-/<=>@^|]/, repeat(OP_CHAR))),

		// Names

		_value_name: ($) =>
			choice(alias($._identifier, $.value_name), $.parenthesized_operator),
		...
		at_mode_expr: ($) => seq("@", $._mode_expr),

		atat_modality_expr: ($) => seq("@@", $.modality_expr),
		...
		mode_legacy: ($) => choice("local_", "unique_", "once_"),
		...
		type_constructor_path: ($) =>
			choice(
				...
				//
				// CR-someday mkeys: Continue searching for a better solution.
				"_",
			),
		...
		_instance_variable_name: ($) =>
			alias($._identifier, $.instance_variable_name),

		_module_name: ($) => alias($._capitalized_identifier, $.module_name),
		_module_type_name: ($) =>
			alias(
				choice($._capitalized_identifier, $._identifier),
				$.module_type_name,
			),
		_constructor_name: ($) =>
			choice(
		...
		directive: ($) =>
			seq(/#/, choice($._identifier, $._capitalized_identifier)),
		type_variable: ($) =>
			seq(/'/, choice($._identifier, $._capitalized_identifier)),
		_type_wildcard: ($) => alias("_", $.type_variable),
		tag: ($) => seq(/`/, choice($._identifier, $._capitalized_identifier)),
		attribute_id: ($) =>
			sep1(/\./, choice($._identifier, $._capitalized_identifier)),
	},
});
